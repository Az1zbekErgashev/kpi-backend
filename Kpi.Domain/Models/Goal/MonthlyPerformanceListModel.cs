...
namespace Kpi.Domain.Models.Goal
{
    public class MonthlyPerformanceListModel : Auditable
    {
        ...
        public virtual MonthlyPerformanceListModel MapFromEntity(Domain.Entities.User.User entity, string year, string month)
        {
            ...
            Status = entity?.CreatedGoals.Where(x => x.CreatedAt.Year == int.Parse(year) && x.IsDeleted == 0)?.FirstOrDefault()?.MonthlyPerformance.Where(x => x.Year == int.Parse(year) && x.Month == int.Parse(month)).FirstOrDefault()?.Status
             ?? GoalStatus.NoWritte;
            Year = year;
            Month = month;
