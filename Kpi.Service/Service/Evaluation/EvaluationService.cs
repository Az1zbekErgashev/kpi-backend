    ...
    public class EvaluationService : IEvaluationService
    {
        ...
        public async Task<IEnumerable<TeamEvaluationResultDto>> GetTeamEvaluationsAsync(EvaluationsForFilterDTO dto)
        {
            ...
            var employees = await _context.Users
                    .Where(e => e.TeamId == teamId && e.Role != Role.TeamLeader)
        ...
        public async Task<IEnumerable<TeamEvaluationResultDto>> GetTeamLeaderEvaluationsAsync(EvaluationsForFilterDTO dto)
        {
            var user = httpContextAccessor?.HttpContext?.User
             ?? throw new InvalidCredentialException();
        ...
        public async ValueTask<object> GetAllEvaluation(int year)
        {
            ...
            var evaluations = await evaluationService.GetAll(x => x.Year == year && x.User.TeamId != null && x.User.RoomId != null && x.User.Role != Role.TeamLeader)
                .Include(x => x.User)
        ...
        public async ValueTask<object> GetAllEvaluationByYear(int year)
        {
            ...
            var evaluations = await evaluationService.GetAll(x => x.Year == year && x.User.TeamId != null && x.User.RoomId != null && x.Status == GoalStatus.Approved && x.User.Role != Role.TeamLeader)
                .Include(x => x.User)
                .ThenInclude(x => x.Position)
                .Include(x => x.User)
                .ThenInclude(x => x.Team)
        ...
        public async ValueTask<object> GetAllEvaluationByYearForTeam(int year)
        {
            var user = httpContextAccessor?.HttpContext?.User
              ?? throw new InvalidCredentialException();
        ...
        public async ValueTask<object> GetEvaluationByYearForTeam(int year)
        {
            var user = httpContextAccessor?.HttpContext?.User
              ?? throw new InvalidCredentialException();
            ...
            var leaderGoal = await goalService.GetAll(x =>
                    x.CreatedAt.Year == year &&
                    x.CreatedBy.Id == userId)
                .Include(x => x.Divisions)
                    .ThenInclude(d => d.Goals.Where(g => g.TargetValue != null && g.TargetValue.Type == TargetValueType.LeaderEvaluation))
                        .ThenInclude(g => g.TargetValue)
                .FirstOrDefaultAsync();

            if (divisions is null) throw new KpiException(404, "goal_not_found");

            var evaluations = await evaluationService.GetAll(x => x.Year == year && x.User.TeamId == teamIdByToken && x.User.RoomId != null && x.User.Role == Role.TeamLeader)
                .Include(x => x.User)
                .ThenInclude(x => x.Position)
                .Include(x => x.User)
                .ThenInclude(x => x.Team)
            ...
            var ceoDivisions = divisions.Divisions
             .Where(d => !string.IsNullOrWhiteSpace(d.Name))
             .Select(d => new
             {
                 Name = d.Name,
            ...
            var leaderDivisions = leaderGoal?.Divisions
                .Where(d => !string.IsNullOrWhiteSpace(d.Name))
                .Select(d => new
                {
                    Name = d.Name,
            ...
            var allDivisionNames = ceoDivisions
                .Where(ceoDiv => leaderDivisions.Any(ld =>
                    ld.Name.ToLower().Trim().Contains(ceoDiv.Name.ToLower().Trim()) &&
                    ld.Ratio == ceoDiv.Ratio))
                .ToList();
